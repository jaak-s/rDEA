...
test_that("input DEA with variable RTS works", {
  dea = dea.input( XREF=X, YREF=Y, X=X[firms,], Y=Y[firms,], RTS="variable" )
...
test_that("input DEA with constant RTS works", {
  dea = dea.input( XREF=X, YREF=Y, X=X[firms,], Y=Y[firms,], RTS="constant" )
  expect_equal( length(dea$thetaOpt), length(firms) )
...
test_that("input DEA with non-increasing RTS works", {
  dea = dea.input( XREF=X, YREF=Y, X=X[firms,], Y=Y[firms,], RTS="non-increasing" )
  expect_equal( length(dea$thetaOpt), length(firms) )
